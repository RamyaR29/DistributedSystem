...
class Calc(object):
    tier3_address = ('localhost', 0xBAD3)
    ...
    def evaluate(tree):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as requester:
            requester.connect(Calc.tier3_address)
            requester.sendall(pickle.dumps(tree))
    ...
    def serve(address):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
            ...
            listener.listen(5)
            print('listening for expressions on {}'.format(listener.getsockname()))
            while True:
                client, client_addr = listener.accept()
                with client:
                    expression = pickle.loads(client.recv(4096))
                    client.sendall(pickle.dumps(Calc.compute(expression)))


if __name__ == '__main__':
    Calc.serve(('localhost', 0xBAD2))
